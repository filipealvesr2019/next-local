...
const ClientChat = ({userName}) => {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        const response = await fetch(`${apiUrl}/api/messages/${storeID}/user/${userID}`);
    ...
    socket.on('adminMessage', (adminMessage) => {
      setChat((prevChat) => [...prevChat, { from: 'Admin', text: adminMessage }]);
    });
  ...
  return (
    <div>
      ...
      <div style={{ border: '1px solid #ccc', height: '300px', overflowY: 'scroll' }}>
        {chat.map((msg, idx) => (
          <div key={idx}>
            <strong>{msg.from}:</strong> {msg.message} {/* Corrigido para usar 'message' */}
          </div>
        ))}
      </div>
      <input
